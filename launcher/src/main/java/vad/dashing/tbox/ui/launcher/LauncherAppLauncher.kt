...
private fun launchWithTarget(
    ...
    // Prefer explicit activity (own MainActivity / multi-activity packages) over package default.
    ...
    // Own classic UI lives in a separate taskAffinity — force a clean new task from HOME.
    if (packageName == context.packageName &&
        activityName == vad.dashing.tbox.MainActivity::class.java.name
    ) {
        intent.addFlags(
...
internal fun launchTBoxSettings(context: Context) {
    // Small floating «О лаунчере» dialog above everything; fall back to the fullscreen
    // MainActivity when the overlay window is unavailable (no SYSTEM_ALERT_WINDOW grant).
    if (LauncherAboutOverlayWindow.show(context)) return
...
private fun startResolved(
    ...
    // Default: freeform in the right-panel zone. Optional per-app fullscreen.
    if (activity != null && isFreeformEnabled(context)) {
        when {
            forceFullscreen -> {
                val bounds = LauncherEmbeddedBoundsState.fullScreenBounds()
                if (bounds != null &&
                    tryLaunchIntentInBounds(activity, packageName, launchIntent, bounds)
                ) {
                    return
                }
            }
    ...
    val fallbackIntent = Intent(launchIntent).apply {
        applyHomeLauncherLaunchFlags(this)
    }
    runCatching { (activity ?: context).startActivity(fallbackIntent) }
        .onSuccess {
            LastAppTracker.recordLaunch(context, packageName)
